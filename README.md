...
Arbeitsergebnisse (Deliverables) und Stakeholder-Signale pro Projekt
zusammenlaufen. Kolleg:innen sehen jederzeit den aktuellen Stand jedes
Projekts (KI-generierte Statuszusammenfassung + Zeitverlauf) und können
entscheidungsreife Deliverables direkt aus dem gesammelten Projektwissen
erzeugen lassen — DSGVO-konform.
...
Teams/Slack/Jira (siehe "Erweiterungsmöglichkeiten" unten).
...
- **Anthropic Claude API** (`@anthropic-ai/sdk`) für:
  - automatische Projekt-Statuszusammenfassungen (Ampel-Status +
    Kurztext) aus dem gesamten Projektwissen,
  - Generierung entscheidungsreifer Deliverables (Memos, Briefings) aus
    dem Projektwissen,
  - Fragen & Antworten über den Wissensbestand eines Projekts (RAG-artig
    durch direktes Einfügen des relevanten Projektwissens in den Prompt,
...
| Tabelle | Zweck |
|---|---|
| `projects` | Projekt-Stammdaten + zuletzt generierte KI-Statuszusammenfassung |
...
1. **Projekt anlegen** auf der Startseite.
2. Im Projekt unter den Tabs **Research / Meetings / Deliverables /
   Stakeholder-Signale** Wissen eintragen (Copy-&-Paste von Notizen,
   Transkripten, Feedback, früheren Ergebnissen).
...
4. Im Tab **Deliverables** ein neues, entscheidungsreifes Dokument aus dem
   gesamten Projektwissen generieren lassen (z.B. Entscheidungsmemo für ein
   Steering Committee).
5. Im Tab **Frage an die KI** gezielte Fragen zum Projektwissen stellen.
...
- Kein Login/Rollenmodell — jede Person mit Zugriff auf die App sieht alle
  Projekte. Für den Produktiveinsatz: Authentifizierung + rollenbasierte
...
- Echte Connectoren statt Copy-&-Paste: Microsoft 365 / Teams (Meetings,
  Chat), Slack, Jira/Linear (Stakeholder-Signale, Tickets) — die
...
- Vektor-basierte Suche statt Volltext-Einfügung, sobald der
  Wissensbestand pro Projekt zu groß für den Kontext eines einzelnen
  Prompts wird.
- Rollenbasierte Zugriffskontrolle + SSO.
